...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('pinky') && conf.TALKING_PINKY !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=162626&key=5XIMfNAKLJZ5KArx&uid=[uid]&msg=[msg]' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=162626&key=5XIMfNAKLJZ5KArx&uid=[uid]&msg=[msg]' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
...
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Zim-BOT features. Turn your account into a ai chatbot!'
    already_on = 'Zim-bot artificial intelligence is already fully functional.'
    already_off = 'Zim-bot artificial intelligence is currently running semi-functional.'
    succ_on = 'Zim-bot Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Zim-bot Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
...
Asena.addCommand({ pattern: 'zimbot ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.zimbot on / off' }, (async (message, match) => {
